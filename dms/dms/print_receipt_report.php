...
function load_perpage(perpage, page, searchByDate){
	window.location = 'print_receipt_report.php?perpage='+perpage+'&page='+page+searchByDate;
}
...
if($_GET['from_date'] <> '' && $_GET['to_date'] <> ''){
	...
	$condition2 = "where invoice.isCancel = '0' and invoice.invoiceDate between '$_GET[from_date]' and '$_GET[to_date]'";
	$searchByDate = "&from_date=$_GET[from_date]&to_date=$_GET[to_date]";
}else{
	$condition = "where isCancel = '0'";	
	$condition2 = "where invoice.isCancel = '0'";	
	$searchByDate = "";
}
  ...
  <div class="output">
    <div class="receipt">
    	<div class="print-header">
        	<div class="header-title"><b>BANGLADESH DIAGNOSTIC & CONSULTATION CENTRE</b></div>
            <div class="address">
                Dhap(Medical More), Jail Road, Rangpur.
            </div>
        ...
        <?php
            if($page == $total_page){
				...
				while($array = $totalQuery->fetch_array()){
					$t_d = $mysqli->query("select sum(disAmount) from patient_discount where invoiceId = '$array[invoiceId]'") or die($mysqli->error);
					$tdis_array = $t_d->fetch_array();
	
					$twriteOff = $mysqli->query("select sum(writeOffAmount) from write_off where invoiceId = '$array[invoiceId]'");
					$twriteOffArray = $twriteOff->fetch_array();
					
					$net = $array['totalTestAmount'] - ($tdis_array[0] + $twriteOffArray[0]);
					
					$totalAmount2 += $array['totalTestAmount'];
					$totalDiscount2 += $dis_array[0];
					$totalNetAmount2 += $net;
				}
				
				/*
				$tcQuery = $mysqli->query("select * from invoice join invoice_tests on invoice.invoiceId = invoice_tests.invoiceId $condition2 and invoice_tests.testId in(select testId from tests where testCategoryId = '30' or testCategoryId = '31')");
				
				$tcQuery1 = $mysqli->query("select * from invoice join invoice_tests on invoice.invoiceId = invoice_tests.invoiceId $condition2 and invoice_tests.testId in(select testId from tests where testCategoryId = '37')");
				*/
        	?>
          	...
          	<tr style="font-weight:bold">
                ...
                <td style="border-bottom:1px solid; border-right:1px solid; border-top:4px double; text-align:center"><?php echo sprintf('%.2f',$totalAmount2); ?></td>
                ...
                <td style="border-bottom:1px solid; border-right:1px solid; border-top:4px double; text-align:center"><?php echo sprintf('%.2f',$totalNetAmount2); ?></td>
                <td style="border-bottom:1px solid; border-right:1px solid; border-top:4px double; text-align:center">&nbsp;</td>
            </tr>
            <!--
            <tr>
            	<td colspan="6">X-Ray : <?php //echo $tcQuery->num_rows; ?></td>
            </tr>
            <tr>
            	<td colspan="6">USG : <?php //echo $tcQuery1->num_rows; ?></td>
            </tr>
        	-->
            <?php
            }
