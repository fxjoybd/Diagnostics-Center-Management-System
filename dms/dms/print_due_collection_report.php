...
function load_perpage(perpage, page, searchByDate){
	window.location = 'print_due_collection_report.php?perpage='+perpage+'&page='+page+searchByDate;
}
...
if($_GET['from_date'] <> '' && $_GET['to_date'] <> ''){
	$condition = "where patient_due_payment.isCancel = '0' and patient_due_payment.patientDuePayDate between '$_GET[from_date]' and '$_GET[to_date]'";
	$searchByDate = "&from_date=$_GET[from_date]&to_date=$_GET[to_date]";
}else{
	$condition = "where patient_due_payment.isCancel = '0'";	
	$searchByDate = "";
}

$pagination_order = $mysqli->query("select * from patient_due_payment join invoice on patient_due_payment.invoiceId = invoice.invoiceId $condition") or die($mysqli->error);
...
?>
  <div class="output">
    <div class="receipt">
        ...
        <?php	
		$query = $mysqli->query("select * from patient_due_payment join invoice on patient_due_payment.invoiceId = invoice.invoiceId $condition order by patient_due_payment.patientDuePayId desc limit $start, $perpage");
        ...
        <table border="0" width="90%" cellspacing="0" style="font-size:12px">
          <tr>
                ...
                <th style="border:1px solid; border-left:none; font-style:italic">Payment Amount</th>
        ...
        <?php
			...
			while($rows = $query->fetch_array()){
				...
				$patientPayment = $mysqli->query("select sum(patientDuePayAmount) from patient_due_payment where invoiceId = '$rows[invoiceId]'") or die($mysqli->error);
			...
			<tr>
                ...
                <td style="border:1px solid; border-top:none; border-left:none; text-align:center">
                	<?php echo $patientPayArray[0]; ?>
                </td>
				<td style="border:1px solid; border-top:none; border-left:none; padding:0px 3px">
					<?php
						$patientPayDate = new DateTime($rows['patientDuePayDate']);
						echo $patientPayDate->format('d-m-Y');
					?>
				</td>
			</tr>
			<?php
			$totalPaid += $patientPayArray[0];
			$sl++;
			}
		?>
        <tr style="font-weight:bold">
        	<td style="border:1px solid; border-top:none; text-align:right" colspan="6">Total=</td>
            <td style="border:1px solid; border-top:none; border-left:none; text-align:center"><?php echo sprintf('%.2f',$totalPaid); ?></td>
