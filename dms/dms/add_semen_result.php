...
<?php
	...
	print_r($_POST);
	echo "</pre>";*/
	$check = $mysqli->query("select * from semen where idNo = '$_POST[idNo]'");
	if($check->num_rows > 0){
		?>
            <center><h3><font color="#FF0000">Invoice <?php echo $_POST['idNo']; ?>'s report allready exist</font></h3></center>
        <?php	
	}else{
	$mysqli->query("insert into semen values('', '$_POST[idNo]', '$_POST[volume]', '$_POST[colour]', '$_POST[liquefacation]', '$_POST[ph]', '$_POST[sperm_count]', '$_POST[actively_motility]', '$_POST[feebly_motility]', '$_POST[non_motility]', '$_POST[s_m_normal]', '$_POST[s_m_abnormal]', '$_POST[pus_cells]', '$_POST[epithelial_cells]', '$_POST[rbc]', '$_POST[sperm_viability]', '$_POST[sperm_clumping]', '$_POST[fructose_test]', '$_POST[opinion]', '$_POST[advice]', '$date', '$time')") or die($mysqli->error);
	...
	//if($addResult){
		...
		?>
        <div class="btn" align="center"><button onclick="window.print()" style="padding:3px 15px; font-weight:bold">Print</button><button onclick="window.location='semen.php?active=semen'" style="padding:3px 15px; font-weight:bold">Back</button></div>
        <div id="main_page">
