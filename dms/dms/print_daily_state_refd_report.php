    ...
    <div class="receipt">
        ...
        <?php
			...
			while($rows = $query->fetch_array()){
				...
				$cashBack = $mysqli->query("select sum(cashBackAmount) from cash_back where invoiceId = '$rows[invoiceId]'");
			...
			<tr>
                ...
                <td style="border:1px solid; border-top:none; border-left:none; text-align:center">
                    <?php echo $pPay = $patientPayArray[0] + $patientDuePayArray[0] - $cashBackArray[0]; ?>
                </td>
                <td style="border:1px solid; border-top:none; border-left:none; text-align:center">
                    <?php echo $patientDue = $net - ($patientPayArray[0] + $patientDuePayArray[0] + $writeOffArray[0] - $cashBackArray[0]);
                    ?>
                </td>
                <td style="border:1px solid; border-top:none; border-left:none; text-align:center">
        ...
        <?php
            if($page == $total_page){
				...
				while($rows2 = $tQuery->fetch_array()){
					...
					$cashBack1 = $mysqli->query("select sum(cashBackAmount) from cash_back where invoiceId = '$rows2[invoiceId]'");
					$cashBackArray1 = $cashBack1->fetch_array();
					...
					$pPay1 = $patientPayArray[0] + $patientDuePayArray[0] - $cashBackArray1[0];
					
					$patientDue1 = $net1 - ($patientPayArray[0] + $patientDuePayArray[0] + $writeOffArray[0] - $cashBackArray1[0]);
      ...
      <?php
        ...
        $cashBack2 = $mysqli->query("select sum(cashBackAmount) from cash_back where isCancel = '0' and cashBackDate between '$_GET[from_date]' and '$_GET[to_date]'");
		$cashBackArray2 = $cashBack2->fetch_array();
      ?>
      <table border="0" width="30%" cellspacing="0" style="font-size: 12pt; font-weight: bold;">
        <tr>
            <td width="55%">Total Cash Received</td>
            <td width="1%">:</td>
            <td><?php echo $totalCashReceived = $payArray[0] + $duePayArray[0] + 0 - $cashBackArray2[0]; ?> Tk.</td>
        </tr>
        <tr>
            <td width="55%">Total Refd Payment</td>
            <td width="1%">:</td>
            <td><?php echo $totalRefdPayment = $refdPayArray[0] + 0; ?> Tk.</td>
        </tr>
        <tr>
            <td width="55%" style="border-top: 1px solid">Total Net Cash</td>
            <td width="1%" style="border-top: 1px solid">:</td>
            <td style="border-top: 1px solid"><?php echo $totalCash = $totalCashReceived - $totalRefdPayment; ?> Tk.</td>
        </tr>
      </table>
