...
<?php
	...
	$check4 = $mysqli->query("select * from bone_marrow_result where idNo = '$_POST[idNo]'");
	if($check4->num_rows > 0){
	?>
    	<center><h3><font color="#FF0000">Invoice ID <?php echo $_POST['idNo']; ?>'s report allready exist</font></h3></center>
    <?php
	}else{
	$mysqli->query("insert into bone_marrow_result values('', '$_POST[idNo]', '$_POST[labNo]', '$_POST[boneMarrow]', '$_POST[boneMarrowContent]', '$date', '$time')");
	
	
	
	//if($addResult){
		$getResult = $mysqli->query("select * from bone_marrow_result join invoice on bone_marrow_result.idNo = invoice.idNo where bone_marrow_result.idNo = '$_POST[idNo]'");
		$metaArray = $getResult->fetch_array();
		?>
        <div class="btn" align="center"><button onclick="window.print()" style="padding:3px 15px; font-weight:bold">Print</button><button onclick="window.location='bone_marrow.php?active=bone_marrow'" style="padding:3px 15px; font-weight:bold">Back</button></div>
        <div id="main_page">
