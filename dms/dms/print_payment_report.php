...
function load_perpage(perpage, page, searchByDate){
	window.location = 'print_payment_report.php?perpage='+perpage+'&page='+page+searchByDate;
}
...
if($_GET['from_date'] <> '' && $_GET['to_date'] <> ''){
	...
	$condition = "where patient_payment.isCancel = '0'";	
	$searchByDate = "";
}

$pagination_order = $mysqli->query("select * from patient_payment join invoice on patient_payment.invoiceId = invoice.invoiceId $condition") or die($mysqli->error);
...
?>
  <div class="output">
    <div class="receipt">
        ...
        <?php	
		$query = $mysqli->query("select * from patient_payment join invoice on patient_payment.invoiceId = invoice.invoiceId $condition order by patient_payment.patientPayId desc limit $start, $perpage");
        ...
        <?php
			...
			<tr>
				...
				<td style="border:1px solid; border-top:none; border-left:none; padding:0px 3px">
					<?php
						$patientPayDate = new DateTime($rows['patientPayDate']);
