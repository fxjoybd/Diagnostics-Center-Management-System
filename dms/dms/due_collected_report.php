...
function report(){
	...
	if(from_date.length > 0 && to_date.length > 0){
		...
		var win = window.open('print_due_collection_report.php?from_date='+from_date+'&to_date='+to_date,'myDoc', myFeatures);	
...
function searchByDate(){
	...
	if(from_date.length > 0 && to_date.length > 0){
		...
		window.location = 'due_collected_report.php?active=receipt_report&from_date='+from_date+'&to_date='+to_date;
	}else{
		alert('Date field empty!');
	}
            ...
            <center>
               ...
               <div class="row">
                  ...
                  <div class="col-lg-12">
					...
					$query = $mysqli->query("select * from patient_due_payment join invoice on patient_due_payment.invoiceId = invoice.invoiceId $condition order by patient_due_payment.patientDuePayId desc");
					if($query->num_rows > 0){
						$sl = 1;
						$price = 0;
						...
						<tr>
                            ...
                            <td>
                            	<?php echo $rows['patientDuePayAmount']; ?>
                            </td>
                            <td>
								<?php
									$invoiceDate = new DateTime($rows['patientDuePayDate']);
