...
function report(){
	...
	if(from_date.length > 0 && to_date.length > 0){
		...
		var win = window.open('print_payment_report.php?from_date='+from_date+'&to_date='+to_date,'myDoc', myFeatures);	
...
function searchByDate(){
	...
	if(from_date.length > 0 && to_date.length > 0){
		...
		window.location = 'refd_payment_report.php?active=refd_payment_report&from_date='+from_date+'&to_date='+to_date;
	}else{
		alert('Date field empty!');
	}
...
function confirmInvoice(id){
		...
		var myOptions = 'scrollbars=no,width=600,height=500,resizeable=no,top=10, left=200,';
		var myFeatures = myBars + ',' + myOptions;
               ...
               <div class="row">
               	<div class="col-lg-12">
                	<div class="form-row show-inputbtns" align="center">
                        ...
                        <button class="btn btn-primary" onclick="">Report</button>
                    </div>
                </div>
               </div>
               </center>
                  ...
                  <div class="col-lg-12">
					...
					if(isset($_GET['from_date']) <> '' && isset($_GET['to_date']) <> ''){
						$condition = "where isCancel = '0' and refdPayDate between '$_GET[from_date]' and '$_GET[to_date]'";
					}else{
						$condition = "where isCancel = '0'";	
					}
					
					$query = $mysqli->query("select * from refd_payment $condition order by refdPaymentId desc");
					if($query->num_rows > 0){
						$sl = 1;
						...
						<tr>
                            ...
                            <td><?php echo $coArray['coCode']; ?></td>
                            <td><?php echo $coArray['coName']; ?></td>
                            <td>
                            	<?php echo $rows['refdPayAmount']; ?>
                            </td>
                            <td>
                            	<?php echo $rows['refdPaymentBy']; ?>
                            </td>
                            <td>
								<?php
									$payDate = new DateTime($rows['refdPayDate']);
									echo $payDate->format('d-m-Y');
								?>
                            </td>
							<td align="center">
