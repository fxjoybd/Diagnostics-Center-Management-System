...
<?php
	...
	$mysqli->query("update semen set volume='$_POST[volume]', colour='$_POST[colour]', liquefacation='$_POST[liquefacation]', ph='$_POST[ph]', sperm_count='$_POST[sperm_count]', actively_motility='$_POST[actively_motility]', feebly_motility='$_POST[feebly_motility]', non_motility='$_POST[non_motility]', s_m_normal='$_POST[s_m_normal]', s_m_abnormal='$_POST[s_m_abnormal]', pus_cells='$_POST[pus_cells]', epithelial_cells='$_POST[epithelial_cells]', rbc='$_POST[rbc]', sperm_viability='$_POST[sperm_viability]', sperm_clumping='$_POST[sperm_clumping]', fructose_test='$_POST[fructose_test]', opinion='$_POST[opinion]', advice='$_POST[advice]' where idNo = '$_POST[idNo]'") or die($mysqli->error);
	
	
	//if($addResult){
		$getResult = $mysqli->query("select * from semen join invoice on semen.idNo = invoice.idNo where semen.idNo = '$_POST[idNo]'");
		$metaArray = $getResult->fetch_array();
		?>
        <div class="btn" align="center"><button onclick="window.print()" style="padding:3px 15px; font-weight:bold">Print</button><button onclick="window.location='semen_report.php?active=semen_report'" style="padding:3px 15px; font-weight:bold">Back</button></div>
        <div id="main_page">
