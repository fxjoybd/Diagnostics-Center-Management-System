...
function searchByDate(){
	...
	if(from_date.length > 0 && to_date.length > 0){
		...
		window.location = 'pbf_report.php?active=pbf_report&from_date='+from_date+'&to_date='+to_date;
	}else{
               ...
               <div class="row">
                  ...
                  <div class="col-lg-12">
					...
					if($_GET['from_date'] <> '' && $_GET['to_date']){
						$condition = "where invoice.isCancel = '0' and pbf_result.date between '$_GET[from_date]' and '$_GET[to_date]'";
					}else{
						$condition = "where invoice.isCancel = '0'";	
					}
					
					$query = $mysqli->query("select * from invoice join pbf_result on invoice.idNo = pbf_result.idNo $condition order by invoiceId desc");
					if($query->num_rows > 0){
						$sl = 1;
						$price = 0;
						...
						<tr>
							...
							<td align="center">
                            <a href="pbf_report_details.php?idNo=<?php echo $rows['idNo']; ?>">Report</a>
                            <a href="pbf_report_edit.php?pbfId=<?php echo $rows['pbfId']; ?>&idNo=<?php echo $rows['idNo']; ?>&active=pbf_report">Edit</a>
                            <a href="pbf_report_delete.php?idNo=<?php echo $rows['idNo']; ?>" onclick="return confirm('Are you sure to Remove this')">Remove</a>
                            </td>
						</tr>
						<?php
						$sl++;
