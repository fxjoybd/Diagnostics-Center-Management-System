...
function report(){
	...
	if(from_date.length > 0 && to_date.length > 0){
		...
		var myOptions = 'scrollbars=no,width=1000,height=500,resizeable=no,top=10, left=200,';
		var myFeatures = myBars + ',' + myOptions;
		
		
		var win = window.open('print_cancel_report.php?from_date='+from_date+'&to_date='+to_date,'myDoc', myFeatures);	
...
function searchByDate(){
	...
	if(from_date.length > 0 && to_date.length > 0){
		...
		window.location = 'cancel_report.php?active=cancel_report&from_date='+from_date+'&to_date='+to_date;
	}else{
		alert('Date field empty!');
	}
